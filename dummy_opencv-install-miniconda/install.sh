...
-D PYTHON3_EXECUTABLE=/usr/local/conda/bin/python \
...
#sudo make -j$nproc
#sudo make install
#sudo ln -s /usr/local/lib/python3.7/site-packages/cv2/python-3.7/cv2.cpython-37m-x86_64-linux-gnu.so /usr/local/lib/python3.7/site-packages/cv2/python-3.7/cv2.so
'
